...
const createRoom = async(req, res, next) =>{
    ...
    try {
        const room = await Hotel.create(req.body)
...
const deleteRoom = async(req, res, next) =>{
    ...
    try {
        ...
        res.status(200).json({message: "Room deletes successfully"})
    } catch (error) {
        res.status(500).json({message:error})
    }
...
module.exports = {getAllRoom, deleteRoom, updateRoom, getDetailRoom, createRoom}